    ...
    def uninstall_for_all_users_in_dism(self):
        try:
            ...
            if provisioned_packages_result.returncode == 740:
                return f"{self.translator.translate('uninstall_for_all_users_in_dism_error_code_740')}\n{provisioned_packages_result.stderr.strip()}"

            # 过滤出带有 Microsoft.MicrosoftPCManager 和 Microsoft.PCManager 的结果
            provisioned_packages = provisioned_packages_result.stdout.splitlines()
            packages_to_remove = [line.split(":")[1].strip() for line in provisioned_packages if "Microsoft.MicrosoftPCManager" in line or "Microsoft.PCManager" in line]

            # 如果有符合的结果，移除预配包
            if packages_to_remove:
                for provisioned_package_name in packages_to_remove:
